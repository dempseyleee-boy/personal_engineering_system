...
class Trial10SpecsTest(unittest.TestCase):
    def test_manifest_and_job_specs_align(self):
        manifest = json.loads(
            (REPO_ROOT / "experiments/round1/runs/dev_trial_010/manifest.json").read_text()
        )
        job_specs = [
            json.loads(line)
            for line in (REPO_ROOT / "experiments/round1/runs/dev_trial_010/job_specs.jsonl").read_text().splitlines()
            if line.strip()
        ]
        self.assertEqual(
            {
    ...
    def test_prompt_contract_exists(self):
        self.assertTrue(
            (
                REPO_ROOT
                / "experiments/round1/prompts/treatment_a_minimal_artifact_example_locked.md"
            ).exists()
        )
